// BarberOS V2 Features - v2.0
(function(){

// Wait for app to be ready
var checkReady = setInterval(function(){
  if(window.db && window.S && typeof openMod === "function"){
...
function overrideFunctions(){
  // Override loadBarbCfg
  window.loadBarbCfg = async function(){
    ...
    var el=document.getElementById("barb-cfg");if(!el)return;
    el.innerHTML=data.map(function(b){
      ...
      var accBtn=hu
        ...
        :'<button onclick="event.stopPropagation();openCreateBarberUser(\x27'+b.id+'\x27,\x27'+nm+'\x27)" style="background:#C9A84C;color:#0E0E0E;border:none;border-radius:50px;padding:4px 10px;font-size:10px;font-weight:700;cursor:pointer">+ Criar acesso</button>';
      return '<div class="ci" style="flex-wrap:wrap;gap:8px">'
        +'<div class="cii">'+(b.avatar_emoji||'\u{1F464}')+'</div>'
        +'<div class="cit" style="flex:1;min-width:120px;cursor:pointer" onclick="editBarber(\x27'+b.id+'\x27,\x27'+nm+'\x27,\x27'+sp+'\x27,'+cm+')">'
  ...
  window.saveBarber = async function(){
    ...
    else{var r=await db.from("barbers").insert({shop_id:S.shopId,name:name,specialty:spec,avatar_emoji:"\u{1F464}",rating:5.0,reviews_count:0,active:true,commission_pct:comm});error=r.error;}
    if(error){toast("Erro: "+error.message,"err");return;}
    toast(eid?"Atualizado!":"Adicionado!","ok");
    closeMod("mod-barber");loadBarbCfg();if(typeof loadBarbers==="function")loadBarbers();
  };
  ...
  window.saveBarberUser = async function(){
    ...
    try{
      var tk=(S.session&&S.session.access_token)||"";
      ...
      closeMod("mod-barber-user");loadBarbCfg();
  ...
  window.filterDash = async function(period){
    ...
    try{
      ...
      if(!ls.length)h+="<div style='color:#9A9080;font-size:12px'>Nenhum atendimento finalizado no per\u00edodo.</div>";
...
function setupBarberLogin(){
  ...
  // Instead, intercept after login by watching S.shopId
  var loginCheck = setInterval(async function(){
    if(!S.shopId || !db) return;
    clearInterval(loginCheck);
    // Check if this user is a barber (not owner)
    try {
      var sess = await db.auth.getSession();
      if(!sess.data.session) return;
      S.session = sess.data.session;
      ...
      // Check if barber
      var barberCheck = await db.from("barber_users").select("*").eq("user_id",uid).maybeSingle();
      if(barberCheck.data){
        S.role = "barber";
        S.barberUserId = barberCheck.data.barber_id;
        applyBarberRestrictions();
      }
    }catch(e){}
  }, 1000);
}

function applyBarberRestrictions(){
  // Hide owner-only tabs (config, services management)
  setTimeout(function(){
    ...
    // In agenda items, hide "Marcar como feito" buttons for barbers
    observeAgendaForBarber();
  },500);
}

function observeAgendaForBarber(){
  // Watch for DOM changes in agenda to hide "done" buttons
  var observer = new MutationObserver(function(){
    if(S.role !== "barber") return;
    // Hide any button that marks as done/finished
    document.querySelectorAll("[onclick*=\"'done'\"]").forEach(function(el){el.style.display="none";});
    document.querySelectorAll("[onclick*=\"'finished'\"]").forEach(function(el){el.style.display="none";});
    document.querySelectorAll("[onclick*=\"'executed'\"]").forEach(function(el){el.style.display="none";});
  });
  var target = document.getElementById("admin-app");
